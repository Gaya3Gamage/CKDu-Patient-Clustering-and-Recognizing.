...
scaler_data = joblib.load('models/scaler_data_top102.sav')
...
def getresults():
	...
	Cs=float(result['Cs'])
	As=float(result['As'])
	S_creatinine=float(result['S_creatinine'])
	Bi=float(result['Bi'])
	Ba=float(result['Ba'])
	Ca=float(result['Ca'])
	Na=float(result['Na'])
	...
	test_data = np.array([V, Hg, Cs, As, S_creatinine, Bi, Ba, Ca, Na, Zn]).reshape(1, -1)
